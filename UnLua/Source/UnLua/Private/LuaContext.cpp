...
void FLuaContext::CreateState()
{
    ...
    if (!L)
    {
		...
		//luacrypt
		if (FLibLuacryptModule::IsAvailable())
		{
			FLibLuacryptModule::Get().SetupLuacrypt(L);
			lua_pushboolean(L, true);
			lua_setglobal(L, "SupportLuacrypt");

			UE_LOG(LogUnLua, Log, TEXT("Lua state setup with Luacrypt."));
		}
